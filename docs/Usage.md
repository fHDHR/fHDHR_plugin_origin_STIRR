...
* this guide assumes we wish to use the `~/fhdhr` directory for our install (you can use whatever directory you like, just make the appropriate changes elsewhere in this guide) and that we are installing for STIRR support
* run the following commands to clone the repo into `~/fhdhr/fHDHR_STIRR`
```
cd ~/fhdhr
git clone https://github.com/fHDHR/fHDHR_STIRR.git
```
* create your config.ini file (as described earlier in this guide) in the `~/fhdhr/fHDHR_STIRR` directory
...
services:
  stirr:
    ...
    network_mode: host
    volumes:
      - ./fHDHR_STIRR/config.ini:/app/config/config.ini
```
* run the following command to build and launch the container
```
docker-compose up --build -d stirr
```
